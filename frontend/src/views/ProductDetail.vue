<template>
  <div class="min-h-screen bg-slate-50">
    <div v-if="product" class="max-w-7xl mx-auto px-4 sm:px-6 py-8">
      <!-- Breadcrumb -->
      <nav class="flex items-center gap-2 text-base text-slate-500 mb-8">
      ...
      <div class="grid lg:grid-cols-2 gap-10 lg:gap-14">
        ...
        <div class="bg-white rounded-2xl border border-slate-200 p-4 shadow-sm">
          <div class="aspect-square rounded-xl overflow-hidden bg-slate-100">
            ...
            >
            <div v-else class="w-full h-full flex items-center justify-center text-8xl bg-gradient-to-br from-slate-100 to-slate-200">
              📦
            </div>
          </div>
        </div>
        ...
        <div class="flex flex-col">
          <div class="flex flex-wrap items-center gap-3 mb-4">
            <span class="badge bg-violet-50 text-violet-700 ring-1 ring-violet-200 text-sm">{{ product.category?.name }}</span>
            <span class="badge bg-emerald-50 text-emerald-700 ring-1 ring-emerald-200 text-sm">
          ...
          <div class="flex items-center gap-1.5 text-amber-400 mt-5">
            ...
            <span class="text-slate-500 ml-2 text-base">({{ product.reviews_count || 0 }} reviews)</span>
          </div>

          <p class="mt-6 text-lg text-slate-600 leading-relaxed">{{ product.short_description || product.description }}</p>

          <div class="mt-8 flex items-baseline gap-4">
            <span class="text-5xl font-extrabold text-slate-900">
              ${{ formatPrice(product) }}
            </span>
            <span v-if="product.sale_price" class="text-2xl text-slate-400 line-through">
          ...
          <div class="mt-8 flex flex-wrap gap-4">
            <button
              ...
              class="bg-violet-600 hover:bg-violet-700 text-white font-bold px-8 py-4 rounded-xl shadow-lg shadow-violet-200 transition flex items-center gap-3 text-lg"
            >
          ...
          <div class="mt-8 grid grid-cols-2 gap-4 text-base">
      ...
      <div class="mt-14 card p-6 sm:p-8">
        <h3 class="text-2xl font-extrabold text-slate-900 mb-8">Customer Reviews</h3>

        <div v-if="isLoggedIn" class="bg-slate-50 rounded-xl p-5 sm:p-6 mb-8 border border-slate-100">
          <h4 class="font-bold text-slate-800 text-lg mb-4">Write a review</h4>
          <div class="flex flex-wrap items-center gap-3 mb-4">
          ...
          <div v-for="review in product.reviews" :key="review.id" class="border border-slate-100 rounded-xl p-5 hover:bg-slate-50/60 transition">
            <div class="flex items-center justify-between mb-3">
              <div class="flex items-center gap-3">
                <div class="w-9 h-9 rounded-full bg-violet-100 text-violet-700 flex items-center justify-center text-sm font-bold">
                  {{ review.user?.name?.charAt(0) }}
                </div>
                <span class="font-bold text-slate-800">{{ review.user?.name }}</span>
              </div>
              <div class="text-amber-400 text-lg">{{ '★'.repeat(review.rating) }}</div>
            </div>
            <p class="text-base text-slate-600 leading-relaxed">{{ review.comment }}</p>
          </div>
        </div>
      </div>
    </div>
    <div v-else class="flex flex-col items-center justify-center py-28 text-slate-500">
      ...
      <p class="font-semibold text-xl">Loading product...</p>
    </div>
  </div>
</template>
